...
function guardarTaxi() {
  ...
  if (!editId) {
    ...
    db.taxis.push({ id: nextId(db.taxis), placa, modelo, estado, creado: today() });
    toast('Taxi agregado ✓', 'success');
  } else {
    Object.assign(db.taxis.find(x => x.id === editId), { modelo, estado });
  ...
  const html = `
    ...
    <div class="card">
      ...
      <div class="filter-row" style="margin-bottom:12px;">
        <input type="date" id="taxi-det-filtro-fecha" oninput="filtrarDetalleTaxi(${taxiId})" placeholder="Filtrar por día" />
