...
// UTILS
// ═══════════════════════════════════════════════
// ahorro ya no es constante — cada registro guarda su propio monto
...
function actualizarMontoDisplay() {
  ...
  const ahorro = parseFloat(document.getElementById('registro-ahorro-monto').value) || 0;
...
function renderDashboard() {
  ...
  const totalHoy = recaudosHoy.reduce((s, r) => s + r.monto + (r.ahorro || 0), 0);
  const totalMes = db.registros.filter(r => r.fecha.startsWith(todayStr.slice(0,7))).reduce((s, r) => s + r.monto + (r.ahorro || 0), 0);
  ...
  if (recaudosHoy.length === 0) {
    ...
    contHoy.innerHTML = recaudosHoy.map(r => {
      ...
      const total = r.monto + (r.ahorro || 0);
  ...
  if (recent.length === 0) {
    ...
    contRecent.innerHTML = Object.entries(byDate).map(([fecha, regs]) => {
      const total = regs.reduce((s,r) => s + r.monto + (r.ahorro || 0), 0);
      return `<div class="recaudo-item">
...
function renderTaxis() {
  ...
  tbody.innerHTML = db.taxis.map(t => {
    ...
    const totalRecaudo = recaudos.reduce((s,r) => s + r.monto + (r.ahorro || 0), 0);
...
function verDetalleTaxi(taxiId) {
  ...
  const totalRecaudo = regsAll.reduce((s,r) => s + r.monto + (r.ahorro || 0), 0);
...
function renderTablaRegsTaxi(regs) {
  ...
  return `<div class="table-wrap"><table>
    ...
    <tbody>${regs.map(r => {
      ...
      const tot = r.monto + (r.ahorro || 0);
      return `<tr><td>${fmtFecha(r.fecha)}</td><td>${c ? c.nombre : '—'}</td><td>${cop(r.monto)}</td>
        <td>${r.ahorro ? `<span class="badge badge-yellow">+${cop(r.ahorro)}</span>` : '—'}</td>
...
function verDetalleConductor(conductorId) {
  ...
  const totalEntregas = regsAll.reduce((s,r) => s + r.monto + (r.ahorro || 0), 0);
  const totalAhorroAcum = regsAll.reduce((s,r) => s + (r.ahorro || 0), 0);
...
function openModalRegistro(id = null) {
  ...
  document.getElementById('registro-ahorro-monto').value = '';
  ...
  if (id) {
    ...
    document.getElementById('registro-ahorro-monto').value = r.ahorro || '';
...
function verDetalleRegistro(registroId) {
  ...
  const html = `
    ...
    <div class="taxi-summary-grid">
      ...
      <div class="taxi-summary-card"><div class="taxi-summary-val" style="color:var(--accent);font-size:18px">${r.ahorro ? cop(r.ahorro) : '—'}</div><div class="taxi-summary-lbl">Ahorro</div></div>
...
function exportarExcel() {
  ...
  taxisEnRegs.forEach(tid => {
    ...
    const totalRec  = regsT.reduce((s,r) => s + r.monto + (r.ahorro || 0), 0);
    ...
    rows.push([
      ...
      cell(regsT.reduce((s,r)=>s+(r.ahorro||0),0), COLOR.totalRec, fontB(11,YELLOW), 'right', COP_FMT, borderBold),
